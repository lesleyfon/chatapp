...
export const ChatRoomSection = ({ data }: { data: [] }) => {
  const [allRoomMessages, setAllRoomMessages] =
    useState<RoomMessagesResponse[]>(data);
  ...
  useEffect(() => {
    ...
    socket.on("add-message-response", (response: RoomMessagesResponse[]) => {
      ...
      if (
        chatroomId?.toString() !== response?.[0]?.chats?.pk_chats_id?.toString()
      ) {
        return;
      }
      ...
      setAllRoomMessages((previousRoomMessages) => {
        response = response.map((responseData) => {
          if (
            responseData?.chat_user?.pk_user_id.toString() === String(userId)
          ) {
